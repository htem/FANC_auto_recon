...
# paths and files
outputpath = '/n/groups/htem/users/skuroda/da'
updated_soma_fname = 'full_VNC_soma_da0820.csv'
...
if getobj == True:
    ...
    LHSda = IDlook.segIDs_from_pts_cv(LHSxyz.astype(int).to_numpy(),cv)
    RHSda = IDlook.segIDs_from_pts_cv(RHSxyz.astype(int).to_numpy(),cv)
    ...
    pMN_T1Ls = connectivity_utils.get_synapses(MNT1L['pt_root_id'],orig_syn_db,direction='inputs',threshold=2)
    pMN_T1Rs = connectivity_utils.get_synapses(MNT1R['pt_root_id'],orig_syn_db,direction='inputs',threshold=2)
...
if getsyn == True:
    ...
    DAN = np.concatenate((left_d, left_a, right_d, right_a)) # np.unique just in case?
    print('descending & ascending neuron table read')

    MN_table = pd.read_csv(MN, header=0)
    print('MN table read')


    # calculate bumber of synapses for each connection
    print('inputs to MNs looking...')
    MN_df = connectivity_utils.get_synapses(MN_table['pt_root_id'],orig_syn_db,direction='inputs',threshold=2)
    MN_inputs = []
    for i in MN_table['pt_root_id']:
    ...
    print('outputs of DA neurons looking...')
    DAN_df = connectivity_utils.get_synapses(DAN,orig_syn_db,direction='outputs',threshold=2)
    DAN_outputs = []
    for i in DAN:

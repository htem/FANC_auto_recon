...
def kimimaro_skeletons(segment_ids,cv_path,cache_path=None,annotations=None,name=None):
    ...
    if name is None:
        name = [str(seg) for seg in segment_ids]
        
    if isinstance(name,str) and len(segment_ids) > 1:
        n = [name + '_' + str(seg) for seg in seg_ids]
        name = n
    ...
    for i in range(len(skels)):
        ...
        skels[i].metadata['name']= name[i]
    
    return(skels)


def get_kimimaro_skeleton(segment_ids,cv_path,cache_path=None,annotations=None,name=None,**format_kwargs):
...
def get_meshparty_mesh(segment_ids,cv_path,cache_path=None,annotations=None,name = None):
...
def meshparty_skeletonize(mesh,
    ...
    # Repair mesh
    mesh.merge_large_components(size_threshold=merge_size_threshold,max_dist=merge_max_dist,dist_step=merge_distance_step)
...
def get_meshparty_skeleton(mesh,
    ...
    swc = mp_to_swc(skeleton,radius=radius,xyz_scaling=xyz_scaling,node_labels=None)

    return(swc)



def mp_to_swc(skeleton,radius=True, xyz_scaling=1,node_labels=None):
    '''
...
def get_cv_mesh(segment_ids,cv_path,cache_path=None,annotations=None,name=None):
